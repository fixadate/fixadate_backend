...
class AdateControllerImplTest extends AdateControllerImplFixture {
	...
	class RestoreAdateTest {
		...
		void 일정을_복구한다() throws Exception {
			...
			mockMvc.perform(
				...
				jsonPath("$.color", is(일정_정보_응답_전달_객체.tag().color())),
	...
	class GetAdatesTest {
		...
		@Test
		void 시작일보다_종료일이_빠른_경우_400을_반환한다() throws Exception {
			// when & then
			mockMvc.perform(
				get("/v1/calendar")
					.with(user(회원_인증_정보))
					.with(csrf())
					.contentType(MediaType.APPLICATION_JSON)
					.queryParam("startDateTime", 종료일.toString())
					.queryParam("endDateTime", 시작일.toString())
			).andExpectAll(
				status().isBadRequest(),
				jsonPath("$.code", is(INVALID_START_END_TIME.getCode())),
				jsonPath("$.message", is(INVALID_START_END_TIME.getMessage()))
			);
		}
	}
	...
	class GetAdatesByMonthTest {
		...
		@Test
		void 월이_12보다_큰_값을_입력할_경우_400을_반환한다() throws Exception {
			// when & then
			mockMvc.perform(
				get("/v1/calendar/month")
					.with(user(회원_인증_정보))
	...
	class UpdateAdateTest {
		...
		void 일정을_수정한다() throws Exception {
			...
			mockMvc.perform(
				...
				jsonPath("$.color", is(일정_수정_결과_응답_전달_객체.tag().color())),
